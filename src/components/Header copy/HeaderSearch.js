...
import UsePublicService from "../../Services/UsePublicgetservice";
...
const HeaderSearch = () => {
  ...
  const { data: Megamenudata } = UsePublicService(
    "getHomePageMappedDetails/publicAccess/getAllCategorySection"
  );
  ...
  async function getProducts(value, from = 0, size = 24) {
    try {
      const data = await Elasticsearch.HeaderSearch(value, from, size);
      setpagination({
        ...pagination,
        total: data.hits.total,
      });
      SetResultData(Elasticsearch.FormatResults(data));
